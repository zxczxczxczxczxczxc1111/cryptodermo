...
- **Исключение — `src/lib/base64.ts`** (`base64ToBytes`/`bytesToBase64`): вынесено по прямому запросу пользователя 19.08.2026, когда пара оказалась скопирована в 11 файлов семнадцатью функциями. Новый код должен импортировать оттуда, а не заводить свою копию. Копия в `vaultStore.test.ts` оставлена намеренно — тест не должен зависеть от кода, который проверяет.
