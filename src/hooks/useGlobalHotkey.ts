 ...
 * принадлежат системе. Для случая «приложение закрыто» существует отдельный
 * ярлык быстрого доступа с флагом `--quick`, и одно другому не мешает: пока
 * программа открыта, срабатывает это сочетание и PIN не спрашивается вовсе
 * (база уже расшифрована), а когда закрыта - ярлык поднимает маленькое окно.
 *
