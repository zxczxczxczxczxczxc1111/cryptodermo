          ...
          >
            {details === "quick" ? (
              <>
                ...
                <p>
                  Окно открывается по сочетанию клавиш поверх любой программы: набираете
                  несколько букв, Enter кладёт пароль в буфер обмена. Shift+Enter копирует
                  логин, у записей с двухфакторкой рядом есть кнопка для кода. Окно
                  остаётся открытым, пока вы его не закроете или пока не пройдёт минута
                  бездействия.
                </p>
                <p>
                  Сочетание задаётся здесь же, выше, и работает, пока приложение запущено.
                  Чтобы оно было под рукой весь день, включите «При закрытии сворачивать в
                  трей»: окно уходит в область уведомлений, программа остаётся в памяти.
                </p>
                <p>
                  Если база к этому моменту успела заблокироваться сама, окно спросит
                  PIN-код прямо в себе и откроет базу, не разворачивая всё приложение.
                  Без настроенного PIN оно этого не умеет: разбор мастер-пароля занимает
